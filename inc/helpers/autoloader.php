...
namespace Gemini_Image_Generation\Inc\Helpers;
...
function autoloader( $resource = '' ) { // phpcs:ignore Universal.NamingConventions.NoReservedKeywordParameterNames.resourceFound
	...
	$resource_path  = false;
	$namespace_root = 'Gemini_Image_Generation\\';
	...
	if ( 'inc' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/inc/%s/%s.php', untrailingslashit( GEMINI_IMG_GEN_FEATURES_PATH ), $directory, $file_name );
	...
	if ( 'tests' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/tests/php/%s.php', untrailingslashit( GEMINI_IMG_GEN_FEATURES_PATH ), $file_name );
...
spl_autoload_register( 'Gemini_Image_Generation\Inc\Helpers\autoloader' );
