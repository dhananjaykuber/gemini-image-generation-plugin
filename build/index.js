...
jQuery(document).ready(function ($) {
  ...
  $(document).on('click', '#gemini-generate-button', function () {
    ...
    $.post(geminiImgGen.ajaxURL, {
      ...
      const $infoMessage = $('<span>').text(__('Click on the image to upload')).attr('class', 'gemini-info-message');
      const $btn = $('<button>').attr('id', 'gemini-image-button').attr('data-title', response.data.title).attr('data-alt', response.data.alt);
      const $img = $('<img>').attr('src', response.data.image).attr('alt', response.data.alt || __('Generated Image')).attr('class', 'gemini-generated-image');
      $btn.append($img);
  ...
  $(document).on('click', '#gemini-image-button', function () {
    ...
    $.post(geminiImgGen.ajaxURL, {
      ...
      const $message = $('<div>').addClass('notice notice-success').text(__('Image inserted successfully! Please refresh the page and check the media library.'));
